...
def main():
    ...
    parser.add_argument("modeler_file", type=str,
                        help="path to a diffBragg modeler file (output from hopper, see the imgs folder in the outdir)")
    parser.add_argument("--scroll", action="store_true",
                        help="if provided, scroll through shoeboxes one-by-one using arrow keys")
    parser.add_argument("--stateFile", type=str,
                        help="Optional path to the roi checker file (requires pytorch). Will be used to label rois as good/bad fits")
    ...
    if not args.scroll:
        ...
        plt.subplot(121)
        plt.gca().set_facecolor('tomato')
        plt.gca().set_title("DATA", fontsize=18)
        masked_dat_vals = full_dat_im[~full_trust_im]
        full_dat_im[~full_trust_im] = np.nan
        plt.imshow(full_dat_im,  cmap='gray_r')
        ...
        plt.gca().set_xticks(mid_x, minor=True)
        plt.gca().set_yticks(mid_y, minor=True)
        plt.gca().grid(1, color='k', ls='--', which='major')
        plt.gca().grid(1, color='k',alpha=0.5,lw=0.5, ls='--', which='minor')
        plt.gca().set_xticklabels([])
        plt.gca().set_yticklabels([])
        plt.gca().tick_params(length=0, which='both')
        dat_ax = plt.gca()
        #gca().set_xlim(0, size_edg*sub_sh[1])
        #gca().set_ylim(0, size_edg*sub_sh[0])
        #gca().set_yticks([])
        plt.subplot(122)
        plt.gca().set_title("MODEL", fontsize=18)
        masked_mod_vals = full_im[~full_trust_im]
        full_im[~full_trust_im] = np.nan
        plt.gca().set_facecolor('tomato')
        plt.imshow(full_im, cmap='gray_r')#gnuplot')
        ...
        #gca().set_xticks([])
        #gca().set_yticks([])
        plt.subplots_adjust(wspace=0, hspace=0, bottom=0.02, top=0.95, left=0.01, right=0.99)
